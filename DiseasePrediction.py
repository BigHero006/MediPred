...
with st.sidebar:
    ...
    st.markdown("---")
    st.warning('⚠️ **Disclaimer**: These ML models are screening tools only. Predictions should never replace professional medical diagnosis. Always consult healthcare providers for clinical decisions.')
...
if select == 'Diabetes Prediction':
    ...
    with st.container(border=True):
        ...
        with col3:
            ...
            BMI = st.number_input('BMI', min_value=0.0, max_value=70.0, value=24.0, step=0.1)

        col4, col5 = st.columns(2)
        with col4:
            DiabetesPedigreeFunction = st.number_input('Diabetes pedigree function', min_value=0.0, max_value=2.5, value=0.3, step=0.1)
        with col5:
    ...
    if predict_button:
        try:
            ...
            # Check if most values are zero (unrealistic input)
            non_zero_count = sum(1 for val in input_data if val != 0)
            if non_zero_count < 3:
                st.warning('⚠️ Most input values are zero. Please enter realistic patient measurements for accurate predictions. Zero values will produce unreliable results.')
                st.stop()
...
if select == 'Heart Disease Prediction':
    ...
    with st.container(border=True):
        ...
        with col3:
            ...
            Thalach = st.number_input('Max heart rate achieved (bpm)', min_value=0, max_value=250, value=140)
...
if select == 'Parkinsons Prediction':
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            MDVP_Jitter_Percent = st.number_input('MDVP jitter (%)', min_value=0.0, max_value=1.0, value=0.01, step=0.0001, format='%.4f')
            MDVP_Jitter_Abs = st.number_input('MDVP jitter (abs)', min_value=0.0, max_value=0.1, value=0.00006, step=0.00001, format='%.5f')

        with col2:
            MDVP_Shimmer = st.number_input('MDVP shimmer', min_value=0.0, max_value=1.0, value=0.025, step=0.001, format='%.3f')
            MDVP_Shimmer_dB = st.number_input('MDVP shimmer (dB)', min_value=0.0, max_value=5.0, value=0.25, step=0.01, format='%.2f')
            Shimmer_APQ3 = st.number_input('Shimmer APQ3', min_value=0.0, max_value=0.1, value=0.012, step=0.001, format='%.3f')
            Shimmer_APQ5 = st.number_input('Shimmer APQ5', min_value=0.0, max_value=0.1, value=0.015, step=0.001, format='%.3f')
            MDVP_APQ = st.number_input('MDVP APQ', min_value=0.0, max_value=0.1, value=0.018, step=0.001, format='%.3f')
            Shimmer_DDA = st.number_input('Shimmer DDA', min_value=0.0, max_value=0.1, value=0.025, step=0.001, format='%.3f')

        with col3:
            NHR = st.number_input('NHR', min_value=0.0, max_value=1.0, value=0.025, step=0.01, format='%.2f')
            HNR = st.number_input('HNR', min_value=0.0, max_value=40.0, value=21.0, step=0.5)
            MDVP_RAP = st.number_input('MDVP RAP', min_value=0.0, max_value=0.1, value=0.005, step=0.0001, format='%.4f')
            MDVP_PPQ = st.number_input('MDVP PPQ', min_value=0.0, max_value=0.1, value=0.006, step=0.0001, format='%.4f')
            Jitter_DDP = st.number_input('Jitter DDP', min_value=0.0, max_value=0.1, value=0.008, step=0.0001, format='%.4f')

        col4, col5 = st.columns(2)

        with col4:
            RPDE = st.number_input('RPDE', min_value=0.0, max_value=1.0, value=0.5, step=0.01, format='%.2f')
            DFA = st.number_input('DFA', min_value=0.0, max_value=1.0, value=0.65, step=0.01, format='%.2f')
            Spread1 = st.number_input('Spread1', min_value=-10.0, max_value=10.0, value=-5.5, step=0.1, format='%.1f')

        with col5:
            Spread2 = st.number_input('Spread2', min_value=0.0, max_value=1.0, value=0.19, step=0.01, format='%.2f')
            D2 = st.number_input('D2', min_value=0.0, max_value=5.0, value=2.1, step=0.1, format='%.1f')
            PPE = st.number_input('PPE', min_value=0.0, max_value=1.0, value=0.19, step=0.01, format='%.2f')
    ...
    if predict_button:
        try:
            ...
            non_zero_count = sum(1 for val in input_data if val != 0)
            if non_zero_count < 8:
                st.warning('⚠️ Most input values are zero. Please enter realistic vocal measurements for accurate predictions. Zero values will produce unreliable results.')
                st.stop()
